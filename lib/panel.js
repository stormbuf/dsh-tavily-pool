 ...
 * @param deps.onKeysAdded - 新密钥落盘之后调用一次，参数是刚加入的记录（含明文
 *   `key`）。它是 `USAGE-8` 在「添加」这条路径上的触发点：新密钥没有余额读数，
 *   因此这一趟必然为它们各问一次官方。**它不参与本次响应**——余额随后经 `/state`
 *   出现在卡片上，而添加操作立刻返回。
 * @returns `{ status, body }`。
...
async function runKeysCommand(payload, deps) {
  ...
  switch (action) {
    case 'add': {
      ...
      const record = await withKeyEdit(pool, () => pool.addKey({ key: plain, label }));
      // 新密钥没有余额读数，因此这一步**必然**会为它问一次官方（`USAGE-8`）。它是
      // 后台发生的，不拖慢这次添加——批量 200 把时那 200 次查询会各自异步进行，
      // 余额随后逐个出现在卡片上，而不是让用户等上几分钟。
      deps.onKeysAdded?.([record]);
    ...
    case 'addBatch': {
      ...
      const added = await withKeyEdit(pool, () => pool.addKeys({ keys: fresh.map((key) => ({ key })) }));
      deps.onKeysAdded?.(added);
      return {
        status: 200,
 ...
 *
 * **成功时把编辑的返回值原样透出**：`add` 与 `addBatch` 要靠它拿到刚写入的记录，
 * 再交给 `USAGE-8` 的余额补齐。失败时一律抛出，因此调用方拿到 `undefined` 只可能
 * 意味着「这次编辑没有返回值」。
 *
 * @param pool - 密钥池；只用于把落盘失败描述成不含环境信息的文案。
 * @param edit - 执行编辑的 thunk。
 * @returns `edit()` 的返回值。
 * @throws {PanelError} 编辑失败时抛出，状态码按失败种类决定。
 */
