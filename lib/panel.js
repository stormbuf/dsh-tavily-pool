...
/**
 * 密钥池的增删改启停排序（`POOL-4`）与批量添加（`POOL-8`）。
 *
...
async function runKeysCommand(payload, deps) {
  ...
  switch (action) {
    ...
    case 'addBatch': {
      ...
      const { fresh, duplicates, received } = parseKeyLines(readString(payload, 'text'), {
      ...
      if (fresh.length > 0) await withKeyEdit(() => pool.addKeys({ keys: fresh.map((key) => ({ key })) }));
      return {
        status: 200,
 ...
 * 内容，把它当成一把空密钥只会得到一条无从理解的报错。
 *
 * **重复的判定拿得到明文，返回的却只有计数。** 池里已有的与本次批内重复的走同一条路：
 * 两者都是「这一行不会带来一把新密钥」，而用户需要的是「几把加进去了、几把没加」，不是
 * 一份他刚刚亲手粘贴过的清单（`POOL-3`：任何出口都只有脱敏形式）。
 *
 * 比较用**逐字相等**，与 `add` 一致：不认前缀、不做大小写折叠。Tavily 将来换个前缀形状时，
 * 这里跟着失效的会是「去重」而不是「添加」——把用户粘进来的东西原样存下，永远比猜它合法要好。
 *
 * @param text - 粘贴的原始文本。
 * @param options - 去重依据。
 * @param options.existing - 池中已有密钥的明文。
 * @returns `{ fresh, duplicates, received }`：待添加的明文、跳过的重复条数、识别到的非空行数。
 */
export function parseKeyLines(text, { existing = [] } = {}) {
  ...
  return { fresh, duplicates, received: fresh.length + duplicates };
}
