...
.dtp-mask{position:fixed;inset:0;z-index:1000;background:var(--dsw-alias-bg-mask-1,#0000004d);display:flex;align-items:center;justify-content:center;padding:24px}
...
`.trim();
    ...
    function TavilyPoolCard(props) {
      ...
      react.useEffect(() => {
        if (ui.batchOpen !== true || typeof document === 'undefined') return undefined;
        const onKeyDown = (event) => {
          if (event.key === 'Escape') patch({ batchOpen: false, batchText: '' });
        };
    ...
    function batchDialog({ t, ui, patch, busy, onSubmit }) {
      ...
      // 没有 DOM 时不渲染（`lib/client.js` 也会在 Node 里被加载与检验）。真实浏览器里
      // 弹框只可能由一次点击打开，因此这条分支在运行时不会被走到。
      if (typeof document === 'undefined') return null;

      // 挂在**这一次动作**上而不是「有没有动作」上：`busy` 是给整个卡片用的布尔，而按钮上
      // 该显示「添加中…」的只有批量添加这一趟（同一时刻也可能有别的动作在飞）。
        ...
        h('div', {
          ...
          h('div', { className: 'dtp-dialog-foot' },
            ...
            h('button', {
              type: 'button',
              className: 'dtp-button dtp-button-primary',
              disabled: working || ui.batchText.trim().length === 0,
              onClick: () => {
                void onSubmit();
              },
