...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** locale 命名空间，也是本卡片词组表的键。 */
    const NS = 'dsh-tavily-pool';

    /**
     * 本插件那条 loader 行的 id。
     *
     * 与 `index.js` 的 `name` 导出、bundle patch 里 `insert` 那一行的 `id` 逐字相同；
     * `lib/constants.js` 的 `PLUGIN_ID` 是同一个字符串的服务端副本。
     */
    const ROW_ID = 'tavily-pool';

    /**
     * 卡片的挂载点与 key（`PANEL-1`）。
     *
     * DSH 0.1.7 起插件配置页挂在**侧栏 Plugins 页的行详情**上，而不是设置页的
     * `settings.plugin.item`：slot 换成 `plugins.row.config`，key 换成 `<包名>#<行 id>`。
     * 旧的 slot 名在 0.1.7 的宿主里已经不存在，而**注册到不存在的 slot 不会报错**——卡片
     * 只是永远不渲染，因此这两个字符串和行 id 一样是硬契约。
     */
    const CARD_SLOT = 'plugins.row.config';
    const CARD_KEY = `${NS}#${ROW_ID}`;
     ...
     * 同一个，说出来只是噪音；超过它，本地前推（每次成功调用 1、2 积分的**估算**）已经可能
     * 让那个数字与官方对不上，而用户多半也想不起来这份读数是几时拿到的。取 5 而不是 1：一分钟
     * 与「刚刚」没有区别。
     *
     * 注意前推用的是估算值，因此这条提示的措辞只说「读数多久没更新」，不说「差了多少」——
     * 差额本身是估出来的，报出来会把一个估算说成事实（`0004`）。
     */
    ...
    const zh = {
      ...
      history: '调用历史',
      historyHint: '最近 {days} 天的调用次数，按本地日期汇总。',
      historySearch: '搜索',
      historyExtract: '抓取',
      ...
      stats: '调用 {calls} · 成功 {successes} · 失败 {failures}',
    ...
    const en = {
      ...
      historyHint: 'Calls over the last {days} days, bucketed by local date.',
      ...
      stats: '{calls} calls · {successes} ok · {failures} failed',
...
`.trim();
     ...
     *
     * `USAGE-8` 让这段空窗有界：读数超过 1 小时、且该密钥下次被用到时会自动重拉，因此
     * 这一行显示的年龄不会无限增长（先前它可以一直涨到用户手动刷新为止）。
     *
    ...
    function TavilyPoolCard(props) {
      ...
      // slot 宿主给两种视图（`PluginConfigViewProps`）：`page` 是行详情页上的配置正文——就是
      // 这张卡片；`summary` 是宿主在**没有描述**时用来补一行的地方，塞进整张卡片显然不合适。
      // 分支放在组件内部而不是包一层：注册的组件就是卡片本身，少一层壳。
      if (props?.view === 'summary') {
        return h('span', { className: 'dtp-meta' }, t('description'));
      }
    ...
    /**
     * 调用历史那一块：按日调用次数曲线 + 最近若干条明细（`14`）。
     *
     * 曲线**自绘 SVG**，不引任何图表库：`PANEL-2` 只允许点种子表里的九个模块，而种子里没有
     * 图表库。自绘的代价是这段几何计算，收益是零构建与零依赖。
     *
     * 两条线分开画，**不共用一套纵轴刻度**：搜索与抓取的次数差得很远（一次会话里搜索通常远多于
     * 抓取，且抓取默认不接管），共用刻度会让少的那条贴着零线看不出来。它们各自按自己的峰值归一。
     *
     * @param t - 翻译函数。
     ...
     *
     * `key` 是 `<包名>#<行 id>`：宿主按它把卡片派发给对应那一行，两者不一致时卡片**根本
     * 不会被渲染**，而且没有任何报错——竞品 `dsh-tavily@0.3.0` 因此出过「整个 Web 卡在
     * Failed to load plugins」。这正是 key 是一个常量、而不是两处字面量的理由。
     *
     * `locale` 仍然声明：slot 宿主据此把 `props.t` 接到本 bundle 注册的词表上，而 `t` 是
     * 语言切换能触发重渲染的唯一途径。
     *
