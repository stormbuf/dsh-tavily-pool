...
/**
 * 按**成功 URL 数**算出这一次该记多少积分（`USAGE-6`）。
 *
 ...
 * 0 if the total successful URL extractions has not yet reached 5 calls」），而上游的累起点
 * 与我们无关。我们要的是「这一次该记多少」，它唯一能从事实算出来，就由这个函数算。
 *
 ...
 * @returns 本次应记的积分；没跨过档位时为 0。
 */
export function extractCreditDelta({ successfulUrls = 0, added = 0, depth } = {}) {
 ...
 *   成功与失败的 URL 数、上游的 `request_id`。**不含积分**——那是累计计数换算出来的，见
 *   {@link extractCreditDelta}。
 * @throws {TavilyError} 失败时抛出。
 */
export async function extractTavily({
  ...
  return {
    ...
    // **不在这里算积分。** 「每 5 个成功 URL」是跨请求累计的，因此只有持有累计计数的那一层
    // （`lib/health.js` 的 `recordSuccess`）才知道这一次该不该记一档。这里只如实交出「这一次
    // 成功了几个 URL」，由它经 `extractCreditDelta` 换算。
    successfulUrls,
