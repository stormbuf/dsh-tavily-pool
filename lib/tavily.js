 ...
 * 出现两处。
 *
 * **两条错误文本走两个字段，因为它们不是同一种事实。** `detail` 只承载**上游自己
 * 给出的、结构化的**错误文本（`{detail:{error}}` 那一层）；`bodyExcerpt` 承载非 JSON
 * 响应的开头片段——那多半是代理、WAF 或 CDN 自己生成的 HTML。`lib/health.js` 的永久
 * 失效判据是在文本上做措辞匹配的，而「一段任意 HTML 里出现了 `expired`」与「上游说
 * 这把密钥过期了」完全不是一回事：前者一旦冒充 `detail`，一次中间层 403 就能把一把
 * 健康密钥永久移出池子。两者都会出现在 `message` 里，因为排障时两条线索都有用。
 */
export class TavilyError extends Error {
   ...
   * @param options.bodyExcerpt - 非 JSON 响应体的开头片段；措辞判定不读它。
 ...
 *
 * 固定发送 `include_usage`：本插件**不读**它（见下），留着它只是让响应形状与官方文档
 * 一致，排障时能对账。
 *
 * **不再从响应里提取 `usage.credits`**（2026-09-20 决定）：积分规则由上游随时可能更改，
 * 插件不再统计自身消耗，面板与设置页只展示 `/usage` 的官方余额。余额前推改用固定估算
 * （见 {@link estimateSearchCredits}）。
 *
 * 取消与超时被合并进同一个 signal，但在抛出的错误里保持可区分，因为二者对调用方
 * 含义不同：被调用方取消的请求必须按取消向上传递，而超时属于密钥层面的失败，
 * 值得再试一次。这件事由 {@link postJson} 自己做——**超时在那里武装**，覆盖从发出
 * 请求到读完响应体的全过程，因此调用方只交自己的取消信号。
 *
 * @param options - 本次搜索。
 ...
 * @returns `{ result, requestId }`：seam 归一化后的结果、上游的 `request_id`。
...
export async function searchTavily({
  ...
  if (!response.ok) {
    // **只有上游信封里的文本才是 `detail`。** 非 JSON 响应体（最常见的是代理/WAF/CDN
    // 自己生成的 HTML）走在 `bodyExcerpt` 上：它照样进 `message` 供人排障，但不参与
    // 任何措辞判定——「这段 HTML 里出现了 expired」不等于「上游说这把密钥过期了」。
    const detail = detailOf(parsed);
    const bodyExcerpt = detail === undefined ? bodyText.slice(0, 300) : undefined;
    throw new TavilyError(
      // `request_id` 是官方「排障时提供给支持」的那个标识，因此只要上游给了就带上
      // （`REST-10`）：错误路径是最需要它的地方，成功路径反而不需要。
      `Tavily returned HTTP ${String(response.status)}: ${detail ?? bodyExcerpt}`
...
/**
 * 估算一次搜索消耗多少积分，**仅用于余额前推**。
 *
 * 官方 `search_depth` 描述：`advanced` 每次计 2 积分，其余三档（`basic` / `fast` /
 * `ultra-fast`）各计 1 积分。
 *
 * ⚠️ 这是**估算**，不是记账（2026-09-20 决定）：插件不再统计自身消耗，也不展示任何
 * 自算的积分数字。这个值唯一的去处是 {@link PoolStore#advanceUsage}——让调度器在两次
 * `/usage` 刷新之间也能按大致余额排序。算错不影响任何展示。
 *
 * @param depth - 搜索深度。
 * @returns 估算的积分消耗。
 */
export function estimateSearchCredits(depth) {
  return depth === 'advanced' ? ADVANCED_SEARCH_CREDITS : BASIC_SEARCH_CREDITS;
}

/**
 * 按**成功 URL 数**估算这一次该算多少积分，**仅用于余额前推**（`USAGE-6`）。
 *
 * 官方的计费口径是「每 5 个成功 URL 计 1 积分（`basic`）/ 2 积分（`advanced`）」，且
 * 「**失败的 URL 永不收费**」。**5 是跨请求累计的**，因此单 URL 的一次抓取绝大多数时候是
 * **0 积分**——只有累计跨过第 5、10、15… 个成功 URL 的那一次才记一档。按请求取整（每抓一次
 * 记 1 积分）会让本地余额以五倍速度下降，而余额正是 `balance` 策略的排序输入。
 *
 * 因此这个函数要的是**累计计数**，不是本次的条数：它比较跨档前后各能记几档。调用方
 * （`lib/health.js` 的 `recordSuccess`）把每次的成功 URL 数累加进密钥统计，再把累加前后的
 * 结果交给这里。
 *
 * ⚠️ 这是**估算**，不是记账（2026-09-20 决定）：积分规则由上游随时可能更改，本插件不再
 * 统计自身消耗、也不展示任何自算的积分数字。这个值唯一的去处是
 * {@link PoolStore#advanceUsage}——让调度器在两次 `/usage` 刷新之间也能按大致余额排序。
 * 算错不影响任何展示，只轻微影响排序。
 *
 * @param options - 累计计数与档位。
 * @param options.successfulUrls - 此前累计的成功 URL 数。
 * @param options.added - 这一次新成功了多少个 URL。
 * @param options.depth - 抽取深度：`basic` 或 `advanced`。
 * @returns 本次估算的积分；没跨过档位时为 0。
 */
export function estimateExtractCredits({ successfulUrls = 0, added = 0, depth } = {}) {
  const before = Number.isInteger(successfulUrls) && successfulUrls > 0 ? successfulUrls : 0;
  const count = Number.isInteger(added) && added > 0 ? added : 0;
  if (count === 0) return 0;
  const perFive = depth === 'advanced' ? ADVANCED_EXTRACT_CREDITS_PER_FIVE : BASIC_EXTRACT_CREDITS_PER_FIVE;
  const tiers = (total) => Math.floor(total / EXTRACT_URLS_PER_CREDIT_TIER);
  return (tiers(before + count) - tiers(before)) * perFive;
}
 ...
 * @param options.fetchImpl - `fetch` 实现。
 * @param options.timeoutMs - 单次尝试的超时。
 * @returns `{ result, successfulUrls, failedUrls, requestId }`：seam 归一化后的结果、这次
 *   成功与失败的 URL 数、上游的 `request_id`。**不含积分**——积分估算由累计计数那一层做，
 *   见 {@link estimateExtractCredits}。
 * @throws {TavilyError} 失败时抛出。
 */
export async function extractTavily({
  ...
  const body = {
    ...
    // 与搜索同理：本插件不读 `usage`，留着它只是让响应形状与官方文档一致，排障时能对账。
    include_usage: true,
  };
  ...
  if (!response.ok) {
    ...
    //
    // 两条来源都必须是**上游信封之内**的文本：`failed_results[]` 与 `detail.error` 都是
    // 上游结构化响应的一部分。非 JSON 响应体的开头片段走 `bodyExcerpt`，理由同
    // {@link searchTavily}。
    const detail = describeFailedResults(parsed?.detail?.failed_results) ?? detailOf(parsed);
    const bodyExcerpt = detail === undefined ? bodyText.slice(0, 300) : undefined;
    throw new TavilyError(
      `Tavily returned HTTP ${String(response.status)}: ${detail ?? bodyExcerpt}`
      + (requestId === undefined ? '' : ` (request_id: ${requestId})`),
  ...
  return {
    ...
    // **不在这里估算积分。** 「每 5 个成功 URL」是跨请求累计的，因此只有持有累计计数的那一层
    // （`lib/health.js` 的 `recordSuccess`）才知道这一次该不该算一档。这里只如实交出「这一次
    // 成功了几个 URL」，由它经 `estimateExtractCredits` 换算。
    successfulUrls,
 ...
 * 一个答案。
 *
 * **超时在这里武装，并且一直有效到响应体读完。** `fetch` 在收到响应头时就兑现，而
 * 体读是它之后的独立一步：上游先发头再挂住、连接在体读期间被重置、调用方在体读期间
 * 取消——这三种失败都发生在 `fetch` 之外。因此体读必须与发请求待在**同一个**
 * `try` 里、走**同一套**翻译：漏掉它，该密钥就不进冷却、`failures` 不增、本次请求
 * 不换下一把密钥、调用历史里没有这次调用，而抛给宿主的会是一个没有 code 的裸异常。
 *
 * @param options - 本次请求。
 ...
 * @throws {TavilyError} 无法完成请求或无法读完响应体时抛出。
 */
async function postJson({ url, apiKey, body, signal, fetchImpl, timeoutMs, what }) {
  ...
  try {
    ...
    // 已经不要这个结果了），后者是密钥层面的失败，值得换一把再试。体读阶段的失败与
    // 发请求阶段的失败走同一套判据——`combined` 对两者的约束是同一个。
