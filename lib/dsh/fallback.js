 ...
 * 凭据缺失的形式响亮失败。`docs/dsh-upgrade.md` 的重适配清单第 5 节盯的就是它。
 */
...
export function officialFetchProvider() {
  customFetchProvider ??= new HttpFetchProvider({ ...OFFICIAL_FETCH_LIMITS });
  return customFetchProvider;
}

/** {@link officialFetchProvider} 的实例；平时是按官方默认值构造的那个，测试经
 * {@link setOfficialFetchProvider} 换成记录调用的桩件。 */
let customFetchProvider;
 ...
 *
 * **键集必须与官方 `resolveOptions` 逐键相同**，因为官方提供方会按名字读它：
 * 少一个键，那条路径就静默地少一件官方会做的事。`recordRequest` 就是这样一个键——它是
 * 官方的一等公民副作用（每次真正发出去的搜索都向会话追加一条
 * `web/deepseek-search-llm-request`，宿主把它列为已知事件类型，并有专门的持久化与迁移
 * 分支），而调用点是 `options.recordRequest?.()`：缺了它一声不响，只是同一个 harness 里
 * 「走 Tavily」与「回落」两条路径产生结构不同的会话。
 *
 * 对账守卫在 `test/dsh-fallback.test.js`：它从官方产物里抽出 `resolveOptions` 并与本
 * 函数的产物逐键比对，因此上游加字段时我们会变红，而不是继续静默漏。
 *
 * @param ctx - 插件 context。
 * @returns 官方提供方要的选项对象。
 */
export function resolveOfficialOptions(ctx) {
  ...
  return {
    ...
    // 官方逐字如此：读**当前**发起者（若有）的会话并追加事件。`agents` 不在本插件
    // `inject` 的三项之内（`lib/dsh/host-services.js` 的实测表），因此必须经
    // {@link readService} 反射读取——而且**在回调里读**：真实搜索时该服务已经 active
    // （实测见 `docs/adr/0003-dsh-coupling-surface.md` 耦合点 19），但组合尚未完成时读到
    // 的是 `undefined`。读取失败与「没有发起者」一样，都按「静默不记」处理：与官方一致，
    // 记录会话不该有能力让一次搜索失败。
    recordRequest: (request) => {
      readService(ctx, 'agents')?.currentInitiator()?.session.append('web/deepseek-search-llm-request', request);
    },
  };
}
...
/**
 * 读官方搜索提供方的条目配置；那个条目属于官方包，本插件只读不写。
 *
 * DSH 0.1.7 起没有「按命名空间取值」这回事了：设置的身份是 profile 条目，读取的唯一公开
 * 途径是 `settings.describe()`——它返回每个**有可编辑字段的**条目的已解析配置，`ns` 就是
 * 条目 id。官方包把自己的条目 id 以 `WEB_SEARCH_DEEPSEEK_SETTINGS_NAMESPACE` 导出，因此
 * 这里不必猜字符串。
 *
 * `describe()` 不传 `redactSecrets` 时值**不脱敏**，这正是本插件需要的：官方提供方的
 * `apiKeyEnv` / `baseURL` / `model` 等都要原样透传。代价是它每次都会遍历全部条目并做一遍
 * 表单投影，因此只该在真的要走回落时调用，而不是每次搜索。
 *
 * @param ctx - 插件 context。
 * @returns 官方条目的已解析配置；条目不存在（官方包被停用）时返回 `undefined`。
 */
function readDeepSeekConfig(ctx) {
  ...
  try {
    ...
    // 条目不存在时 `find` 给出 `undefined` 而不是抛错，因此「官方包被停用」这一档自然落进
    // 调用方的默认值路径。留着 `try` 是为了别的原因：settings 是一个自有实现可能变形的
    // 可选服务，而回落路径不该因为一个读配置的意外而整个失败。
    return describe.call(settings).find((entry) => entry.ns === WEB_SEARCH_DEEPSEEK_SETTINGS_NAMESPACE)?.value;
  } catch {
    return undefined;
  }
}
