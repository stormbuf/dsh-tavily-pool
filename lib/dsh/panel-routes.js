...
async function panelState(ctx, state) {
  ...
  return readPanelState({
    settings: readPluginSettings(state),
    pool: state.pool,
    history,
    // **当场探测**而不是读一份加载期缓存：三项注入服务要到 profile 组合完成之后才可见
    // （实测时序见 `index.js`），缓存下来的那份会在插件刚加载完的那几秒里谎报缺失。
    // 探测必须走 `state.host`（已绑定的服务视图），不能走插件本体的 ctx：`settings` 与
    // `connection` 在那上面**永远**读不到，于是面板会一直谎报它们缺失。
    capabilityReport: probeCapabilities({ ctx: state.host }),
...
function panelDeps(ctx, state) {
  return {
    pool: state.pool,
    readSettings: () => readPluginSettings(state),
    writeSettings: (patch) => writePluginSettings(state, patch),
    refresh: state.usageRefresher === undefined
      ? undefined
      : (id, key, options) => state.usageRefresher.refresh(id, key, options),
    // 添加密钥之后顺手补齐它们的余额（`USAGE-8`）。**后台发生**：它不 await，因此
    // 批量添加 200 把时那次请求立刻返回，200 次 `/usage` 各自异步进行，余额随后
    // 逐个出现在卡片上——而不是让用户对着一个转圈的面板等上几分钟。
    onKeysAdded: state.balanceRefresh === undefined
      ? undefined
      : (records) => state.balanceRefresh.sweep(records, { reason: 'added' }),
  };
}
