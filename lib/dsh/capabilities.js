...
export function probeCapabilities(host) {
  ...
  // First, because everything below depends on it: the reflective read is how
  // these services are reached at all, so a host without it would otherwise
  // report "all capabilities present" and then fail inside registration.
  check(
    'ctx.reflectiveRead',
    isFunction(ctx?.get),
