...
/**
 * 一次 Tavily 调用的**总**时间预算，单位毫秒（`SCHED-9`）。
 *
 * 它是 `dsh-tool-web` 的 `searchTimeoutMs`，也是本插件排布等待与尝试的依据：单次尝试
 * 的超时按剩余预算折算，等待预算从它里面分。抓取没有对应的宿主预算常量，沿用同一个
 * 值——抓取的超时更紧（{@link TAVILY_TIMEOUT_MS}），折算出来的尝试次数只会更少，
 * 不会更危险。
 *
...
/**
 * 计费分档的除数：每 5 个成功抽取的 URL 记一档积分（`USAGE-6`）。
 *
...
/** `basic` 档每 5 个成功 URL 的积分（`USAGE-6`）。 */
export const BASIC_EXTRACT_CREDITS_PER_FIVE = 1;

/** `advanced` 档每 5 个成功 URL 的积分（`USAGE-6`）。 */
