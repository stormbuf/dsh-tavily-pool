...
/**
 * 本插件那条 loader 行的 id，同时就是设置表单的**条目 id**。
 *
 * 三处必须是同一个字符串：`index.js` 的 `name` 导出、bundle patch 里 `insert` 那一行的
 * `id`、以及 `settings.update(entryId, …)` 的入参。DSH 0.1.7 起宿主的设置模型以 profile
 * **条目 id** 为身份（`SettingsNamespace` 就是「某个 profile 条目的名义 id」），旧的
 * 「插件自选命名空间」已不存在，因此这个名字不再由本插件随便取。
 */
export const PLUGIN_ID = 'tavily-pool';

/**
 * 本插件的 npm 包名。
 *
 * 客户端卡片挂 `plugins.row.config` 时用它拼出 key：新契约的 key 是
 * `<包名>#<行 id>`，而不是旧契约里的设置命名空间。
 */
export const PACKAGE_NAME = 'dsh-tavily-pool';
...
/**
 * 一次官方读数被视为**超龄**的年龄，单位毫秒（`USAGE-8`）。
 *
 * 超过它，该密钥在下一次真的被用到时会被顺手刷新一次。1 小时是「排序够新鲜」与
 * 「绝不靠近官方限流」之间的取值：单把密钥最坏 1 次/小时，而官方限流是 10 次/10 分钟
 * （即 60 次/小时），相差两个数量级，因此这个阈值下撞不上限流。
 *
 * **它同时把余额前推的误差钉成有界的**：前推（{@link estimateSearchCredits} 那一族）
 * 只在两次官方读数之间累积，而这段空窗现在最多 1 小时。没有这条时，误差会一直累积到
 * 用户手动点一次刷新为止——那可能是几天。
 *
 * **只在插件被真正使用时才判它**（`USAGE-8`）：没有任何定时器，闲置的插件一次
 * `/usage` 都不会发。
 */
export const USAGE_REFRESH_AFTER_MS = 3600 * 1000;
