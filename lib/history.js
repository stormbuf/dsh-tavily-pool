 ...
 * @property {number|undefined} credits - 这次调用**记了多少**积分；`undefined` 表示未知（`REST-3`）。
...
export function normalizeRecord(entry, { nowMs = Date.now() } = {}) {
  ...
  // 抓取这一次成功了几个 URL：它是计费的原始事实，而 `credits` 只记「这一次跨档记了几分」。
  // 少了它，用户看到一条 `credits: 0` 的抓取记录时无法分辨「没跨档」与「什么都没抓到」。
