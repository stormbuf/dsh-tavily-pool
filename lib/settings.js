...
import { EXTRACT_DEPTH_VALUES, EXTRACT_FORMAT_VALUES } from './constants.js';
 ...
 * 抓取接管开关的默认值（`CFG-2`）。
 *
 * 与搜索开关**各自独立**：两个开关谈的是两条不同的路径，共用一个值会让「我只想接管
 * 搜索」这件事无从表达。
 *
 * **默认关闭**（2026-09-20 决定）。搜索与抓取走的是两条不同的上游路径，抓取由
 * Tavily `/extract` 承担，其计费与额度口径独立于搜索；用户装本插件通常是为了搜索，
 * 让抓取默认跟随接管等于替用户做了一个他没要求的额度消耗决定。要接管抓取的人显式
 * 打开它，这个选择留在用户手里。
 *
 * **只改默认值，不改已存配置。** 已经显式写过 `fetchEnabled` 的用户保留自己的选择；
 * 默认值只作用于从未设置过该项的配置。
 */
export const FETCH_ENABLED_DEFAULT = false;
...
/**
 * 本插件的设置形状，同时就是本插件那条 loader 行的 `Config`。
 *
 * 每一项都带 `default`，因为宿主把「schema 默认值 → 组合 base → 用户层」依次解析后
 * 交回一个**完整**的对象；少了默认值，用户没碰过的项在解析结果里根本不存在，面板也
 * 就渲染不出一个可改的初值。
 *
 * 每一项都带 `volatile()`（DSH 0.1.7 起的硬要求）：宿主用 `volatileForm()` 决定哪些
 * 字段进设置表单——根 schema 会递归进 `dict`，只保留 `.volatile()` 的子项，一个都没有
 * 的条目**根本不进 `describe()`**，也就没有可编辑的表单。它同时是「这个字段可以在运行
 * 期改」的声明，与 `applies: 'live'` 对应。
 *
 ...
 * 从**已解析的条目配置**读出当前设置。
 *
 * DSH 0.1.7 起设置就是本插件那条 loader 行的 `config`：loader 按上面的 schema 校验
 * 并补齐默认值，`apply(ctx, config)` 收到的就是它。旧模型里的 `ctx.settings.get(ns)`
 * 已经不存在。
 *
 * **仍然逐项复校验一次，而不是直接信任。** config 来自 profile patch
 * （`~/.dsh/profiles/<p>/cordis.patch.yml`）——一份用户可以直接编辑的文件。一个越界的
 * `maxResults: 21` 会被原样发给 Tavily 并换回一个 `400`；按 `REST-8`，那既不重试也不
 * 切换密钥，等于每一次搜索都注定失败。退回默认值至少让搜索继续工作，这正是 `PIN-5`
 * 的半坏仍可用语义。返回的对象按模块里声明的默认值逐项补齐，因此调用方不必再判
 * `undefined`——一个缺失的字段与一个显式写成默认值的字段，对搜索而言是同一件事。
 *
 * @param config - loader 解析后的条目配置，或任何形状可疑的同类对象。
...
/** 取配置对象本身；不是对象时返回 `undefined`，让每一项各自退回默认值。 */
function readConfig(source) {
  return source !== null && typeof source === 'object' ? source : undefined;
}
