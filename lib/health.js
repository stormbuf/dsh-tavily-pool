...
import { extractCreditDelta } from './tavily.js';
 ...
 * 见 `.scratch/dsh-tavily/Tavily-extract-取证.md` §B.6）。因此 `endpoint: 'extract'` 时那两个码
...
export class KeyHealth {
  ...
  /**
   * 记录一次成功，并按本次消耗**前推余额缓存**（`USAGE-5`）。
   *
   * 成功即清除冷却：这把密钥刚刚被上游接受了，任何关于它「暂时不可用」的记录都已
   * 过时。额度耗尽的标记不在这里清除——`SCHED-8` 要求只由 `/usage` 确认恢复。
   *
   * **前推与记账是两件事，两件都要做。** 记账（`stats.credits`）是流水，供面板展示
   * 「这把密钥一共花了多少」；前推（`usageCache.key.usage`）改的是**调度真正读的那份
   * 余额**（`balanceRank` 读 `usageCache`）。只记账不前推的话，额度充足的密钥会一直被
   * 排在前面：本地明明知道刚花掉 2 积分，排序却仍按上一次 `/usage` 的旧数字来，直到
   * 用户手动点一次刷新。
   *
   * 前推本身由 {@link PoolStore#advanceUsage} 判断可行性（没有缓存、无限额度、值不可用
   * 时都不动）。这里只负责「成功且知道消耗了多少」这个前提。
   *
   * **抓取的消耗在这里算，不在内核里算。** 官方按「每 5 个成功 URL」计费，而 5 是**跨请求
   ...
   * @param outcome.extractUrls - 这次抓取成功了多少个 URL。给了它就**按累计档位**算这次该记
   *   多少积分，而**不读** `credits`（上游那个数按它自己的累起点取整，与我们无关）。
   * @param outcome.extractDepth - 抽取深度，决定每档记 1 还是 2 积分。仅在给了
   *   `extractUrls` 时参与计算。
   * @param outcome.durationMs - 本次耗时。
   * @param outcome.nowMs - 当前时刻。
   * @returns 落盘完成，并给出**本次记了多少积分**：搜索是上游回传的那个数，抓取是按累计档位
   *   算出来的那个数；消耗未知时为 `undefined`。调用方（`lib/attempts.js`）把它交给调用历史。
   */
  async recordSuccess(id, { credits, extractUrls, extractDepth, durationMs, nowMs = this.#now() } = {}) {
    const fetched = Number.isInteger(extractUrls) && extractUrls >= 0;
    // 抓取路径上「消耗」由累计档位算出来，因此它是**已知**的（哪怕这次记 0）；搜索路径上
    // 仍然只有上游回传才算已知（`REST-3`）。
    const known = fetched || (typeof credits === 'number' && Number.isFinite(credits));
    let billed;
    ...
    await this.#merge(id, (stats) => {
      ...
      if (fetched) {
        ...
        billed = extractCreditDelta({ successfulUrls: before, added: extractUrls, depth: extractDepth });
        next.extractUrls = before + extractUrls;
        next.credits = (stats.credits ?? 0) + billed;
    ...
    // 消耗未知时不前推：不知道扣了多少却去猜一个数字，等于把一份本地估计写成官方读数。
    // 抓取那次即使 `billed === 0` 也是**已知的零**，前推 0 由 `advanceUsage` 自行忽略。
    if (known) await this.#pool.advanceUsage(id, billed ?? 0);
    return billed;
  }
