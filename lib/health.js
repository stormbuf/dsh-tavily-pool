...
export class KeyHealth {
  ...
  /**
   * 记录一次成功，并按上游回传的消耗**前推余额缓存**（`USAGE-5`）。
   *
   ...
   * @returns 落盘完成的 promise。
   */
  async recordSuccess(id, { credits, durationMs, nowMs = this.#now() } = {}) {
    const known = typeof credits === 'number' && Number.isFinite(credits);
    ...
    if (known) await this.#pool.advanceUsage(id, credits);
  }
