 ...
 *
 * A key too short for the usual prefix and suffix to fit without overlapping is
 * masked whole rather than partly revealed: there is no way to show part of it
 * safely, and a short string is exactly the case where a "prefix" would be most
 * of the key.
 *
...
export class PoolStore {
  ...
  async update(mutate) {
    ...
    // Serialize unconditionally. The chain is always settled to `undefined`
    // below, so it can never reject into the next caller — which is what makes
    // a plain `then(run)` correct here.
