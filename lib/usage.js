 ...
 * 3. **什么时候该问**——`SCHED-10` 的月起始探测窗口（{@link shouldProbeAfterMonthStart}）
 *    与 `USAGE-8` 的读数超龄判据（{@link needsBalanceRefresh}）。
 *
 ...
 * `lib/health.js`。永久失效标记的撤销同在这条路径上（一次成功的 `/usage` 就是「官方确认
 * 这把密钥还能用」），但**不看余额**：余额为零说的是额度耗尽，那是另一个标记。
 *
 ...
 *
 * 三态，与调度器排序用的是**同一份判定**（`lib/balance.js`）：
 *
 * - **无限**（密钥级与账号级都没有上限）→ **必然为正**。
 * - **已知**且 `remaining > 0` → 正。
 * - **未知**（读不出上限或用量）→ **不能确认为正**。
 *
 ...
 *
 * `20` 之前这里写的是 `key.limit === null → true`：它对免费账号**恰好**给出了正确的
 * 结论（额度确实还在），理由却是错的——那种响应里上限在 `account.plan_limit`。现在结论
 * 仍然对，而理由与调度、前推、面板三处一致。
 *
...
export function hasPositiveBalance(usageEntry) {
  ...
  if (balance.kind === BALANCE_KINDS.unlimited) return true;
  if (balance.kind === BALANCE_KINDS.known) return balance.remaining > 0;
  return false;
}
...
export async function fetchUsage({ apiKey, signal, fetchImpl, timeoutMs = USAGE_TIMEOUT_MS }) {
  ...
  if (!response.ok) {
    // 与 `lib/tavily.js` 同一条约定：`detail` 只承载上游信封里的文本，非 JSON 响应体的
    // 开头片段走在 `bodyExcerpt` 上（两者都会出现在 `message` 里）。这条刷新路径不做
    // 措辞判定，但两个字段的含义在整个模块里只能有一个答案。
    const detail = detailOf(parsed);
    const bodyExcerpt = detail === undefined ? bodyText.slice(0, 300) : undefined;
    throw new TavilyError(
      `Tavily usage query returned HTTP ${String(response.status)}: ${detail ?? bodyExcerpt}`
...
export class UsageRefresher {
   ...
   * @param options.health - 健康状态，提供 `clearQuotaExhausted()` 与
   *   `clearPermanentlyInvalid()`：一次成功的 `/usage` 同时撤销这两个标记，因为它们都
   *   由「上游说不行」而来，而这次上游说的是行。
  ...
  async refresh(id, apiKey, { signal, reason = 'manual' } = {}) {
    ...
    try {
      // **先**落地余额、**再**清标记：反过来的话，清除标记的那一瞬间调度器可能已经看到
      // 「可选中」，并把请求发给了一把官方刚刚说没量的密钥。
      //
      // 额度耗尽只清「能确认余额为正」的那一种（`SCHED-8`）：读不出余额的响应什么也没有
      // 确认，而把一把没量的密钥放回候选，代价是此后每一次搜索都白撞一次 432。
      ...
      // 永久失效**无条件**清：官方读通了这次 `/usage`，这直接反驳了「这把密钥不可再用」
      // 那条结论，而它与余额是多少无关——余额为零说的是额度耗尽，那是另一个标记。
      await this.#health.clearPermanentlyInvalid(id);
...
/**
 * 判断一把密钥的余额缓存现在是否该刷新（`USAGE-8`）。
 *
 * 三种情形要刷新，其余都不动：
 *
 * 1. **从未读到过余额**（没有缓存项）——它此刻在调度里按「未知」垫底（`SCHED-2`），
 *    而未知不是零；补一次读数能立刻让它参与正常排序。
 * 2. **有缓存但读不出读数时刻**（`fetchedAt` 缺失、被手工改坏）——年龄无从判断，
 *    而一份无法定年的读数与没有读数，在「能不能信」上是同一件事。
 * 3. **读数超龄**（年龄超过 {@link USAGE_REFRESH_AFTER_MS}）——排序要用它，而前推
 *    只在两次官方读数之间累积，因此这条同时把估算误差钉成有界。
 *
 * **这是纯判据，不发请求、不看配额**：能不能真的发出去由 `UsageRefresher.refresh`
 * 的滑动窗口预占回答（`USAGE-2`），而「该不该问」与「还能不能问」是两个问题。
 *
 * **它只在插件被使用时被调用**（添加密钥、或某把密钥真的被选中），因此闲置的插件
 * 不会产生任何 `/usage` 流量。
 *
 * @param options - 判断所需的事实。
 * @param options.usage - 该密钥的余额缓存项；从未刷新过时省略。
 * @param options.nowMs - 当前时刻。
 * @param options.maxAgeMs - 超龄阈值。
 * @returns 该刷新时返回 true。
 */
export function needsBalanceRefresh({ usage, nowMs, maxAgeMs = USAGE_REFRESH_AFTER_MS }) {
  if (usage === null || usage === undefined) return true;
  const fetchedAtMs = toMillis(usage.fetchedAt);
  if (fetchedAtMs === undefined) return true;
  return nowMs - fetchedAtMs > maxAgeMs;
}

/**
 * 判断一把密钥**有没有**一份可用的余额读数（`USAGE-8`）。
 *
 * 与 {@link needsBalanceRefresh} 的差别只在「超龄」那一档：读数旧了仍然算「有」。
 * 这个区别对应两条不同的触发路径：
 *
 * - **全池扫描**（每次真的用到插件时）只补**从来没有过读数**的密钥。它们的余额在调度里
 *   是「未知」而垫底（`SCHED-2`），补一次就能让它们参与正常排序。旧读数不在此列：为一把
 *   余额已知、只是读数偏旧的密钥反复发问，是拿官方配额换一点点排序精度。
 * - **被选中的那把**另有超龄规则（{@link needsBalanceRefresh}），因为排序正要按它的余额
 *   做决定。
 *
 * @param usage - 该密钥的余额缓存项；从未刷新过时省略。
 * @returns 有一份能定年的读数时返回 true。
 */
export function hasBalanceReading(usage) {
  if (usage === null || usage === undefined) return false;
  return toMillis(usage.fetchedAt) !== undefined;
}
