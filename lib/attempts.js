 ...
 * @param options.operation - 文案里的动词（`search` / `fetch`），与 `lib/tavily.js` 的
 *   `postJson` 同一个用意：说「search」而实际做的是抓取，会把读日志的人带偏。
...
export async function runWithFailover({
  ...
  while (attemptsMade < maxAttempts) {
    ...
    if (signal?.aborted === true) {
      throw new TavilyError(`Tavily ${operation} aborted by the caller`, { code: 'TAVILY_ABORTED' });
    }
