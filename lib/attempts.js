...
import { WAIT_BUDGET_MS, WAIT_BUDGET_SHARE } from './constants.js';
 ...
 * @param options.invoke - 用给定密钥执行一次真实调用，返回 `{ result }`；
 ...
 *   按没有候选处理。省略表示不做自动探测。
 * @param options.onAttempt - 每次尝试**结束**时调用一次（成功与失败都会调用），携带该次尝试的
 *   事实。`14` 用它记调用历史。它**不参与**任何决策，也不被等待：记录不该拖慢故障切换。
 *   它抛出的东西被吞掉——观察者不该有能力把一次调用搞挂。
 * @param options.signal - 调用方取消信号。
 * @param options.deadlineMs - 本次搜索的截止时刻（`SCHED-9` 的总预算）。
 * @param options.endpoint - 这次调用打的是哪个端点（`search` / `extract`）。它同时是文案里的
 *   动词、也是「哪张错误表适用」的判据（`FETCH-5`）——与 `lib/tavily.js` 用同一个词，
 *   于是「说 search 而实际做的是抓取」这类错位在类型上就不可能。
 * @param options.extractDepth - 抓取深度（`basic` / `advanced`），决定每 5 个成功 URL 估算
 *   1 还是 2 积分。搜索路径上省略。
 * @param options.searchDepth - 搜索深度（`basic` / `advanced` / `fast` / `ultra-fast`），决定
 *   本次按 1 还是 2 积分估算。抓取路径上省略。两个深度都只用于**余额前推的估算**，
 *   不写进任何展示（2026-09-20 决定）。
...
export async function runWithFailover({
  ...
  while (attemptsMade < maxAttempts) {
    // **取消排在最前。** 调用方已经不要这个结果了，因此连一次调度都不该做：那条路径会在
    // 「池内没有候选」时给出一个关于池子的结论，而它答的不是用户问的问题——他要的是「停下来」。
    if (signal?.aborted === true) {
      throw new TavilyError(`Tavily ${endpoint} aborted by the caller`, { code: 'TAVILY_ABORTED' });
    }
    ...
    try {
      ...
      const { classification, persisted } = health.recordFailure(id, {
        failure: failureFactsOf(error, endpoint),
      ...
      lastError = error;
      // 记历史**不等待**：它是记录而不是正确性前提，因此与健康状态落盘并行，不拖慢
      // 「换下一把再试」这条路径。它抛出的东西也一并吞掉：观察者不该有能力把一次搜索搞挂
      // ——这正是「半坏仍可用」在这一处的形状（`PIN-5`）。
    ...
    const durationMs = now() - startedAt;
    // 抓取路径把「这次成功了几个 URL」与深度一并交下去：官方按「每 5 个成功 URL」计费，而 5
    // 是**跨请求累计**的，因此只有持有累计计数的那一层（`lib/health.js`）能估算这次该算多少。
    // 搜索路径交深度，由同一层按 1 / 2 积分估算。
    //
    // **估算只用于余额前推，不写进历史**（2026-09-20 决定）：插件不再统计自身消耗。
    await health.recordSuccess(id, {
      successfulUrls: outcome?.successfulUrls,
      extractDepth,
      searchDepth,
      durationMs,
    });
    attempts.push({ keyId: id, status: 200, action: 'success' });
    // 成功与失败走同一条落盘纪律：`recordSuccess` 内部已经把状态写进内存（下一次调度立刻
    // 读得到），这里 `await` 的是**持久化**本身——否则进程在紧接着的瞬间退出，用户看到的
    // 是一把「没有成功过」的密钥。
    notifyAttempt(onAttempt, {
      keyId: id,
      outcome: 'ok',
...
/**
 * 通知一次尝试已结束；观察者抛出的东西被吞掉。
 *
 * 这一层 `try` 不是防御性编程，而是一条边界：`onAttempt` 是**观察者**，它记的是历史，而历史
 * 是记录而不是正确性前提。少了它，一段写日志或写历史的代码就能让一次搜索失败——那正是
 * `PIN-5` 的「半坏仍可用」不允许的形状。
 *
 * @param onAttempt - 观察者；缺席时什么都不做。
 * @param attempt - 那次尝试的事实。
 */
 ...
 * @param endpoint - 这次调用打的是哪个端点（`search` / `extract`），决定哪张错误表适用。
 * @returns `{ status, detail, code, retryAfter, endpoint }`。
 */
function failureFactsOf(error, endpoint) {
  return {
    status: error.status,
    detail: error.detail,
    code: error.code,
    retryAfter: error.retryAfter,
    // 端点决定哪张错误表适用（`FETCH-5`）：`/extract` 没有 403、没有 422。少了这一项，
    // 抓取路径上的一个代理 403 会被套上「措辞命中即永久失效」而白扔掉一把好密钥。
    endpoint,
  };
}
...
export function waitAllowance(deadlineMs, nowMs) {
  const cap = nowMs + WAIT_BUDGET_MS;
