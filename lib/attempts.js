...
import { FAILURE_ACTIONS } from './health.js';
...
export async function runWithFailover({
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    if (selection.key === undefined) {
      ...
      // 它们（`REST-10`）——它带着上游的 request_id 与状态码。
      if (lastError !== undefined) throw withAdvice(lastError, exhaustedAdvice(tried, health));
      throw new TavilyError(messageFor(selection), { code: 'TAVILY_NO_USABLE_KEY' });
    }
...
function exhaustedAdvice(tried, health) {
  ...
  if (exhausted > 0) {
    parts.push(
      `${String(exhausted)} key(s) in the pool have no credits left for this billing cycle. Credits reset on `
      + 'the 1st of each month regardless of the billing date; to search before then, increase the limit '
      + 'in the Tavily dashboard (https://app.tavily.com/account/plan).',
    );
  }
  if (invalid > 0) {
    parts.push(
