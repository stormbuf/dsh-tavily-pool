 ...
 * @param options.extractDepth - 抓取深度（`basic` / `advanced`），决定每 5 个成功 URL 记 1 还是
 *   2 积分。搜索路径上省略。
...
export async function runWithFailover({
  ...
  while (attemptsMade < maxAttempts) {
    ...
    // 是**跨请求累计**的，因此只有持有累计计数的那一层（`lib/health.js`）能算出这次该记多少。
    // 它把算出来的数作为兑现值交回来，供调用历史使用。
    const persisted = health.recordSuccess(id, {
      credits: outcome?.credits,
      extractUrls: outcome?.successfulUrls,
      extractDepth,
      durationMs,
    });
    ...
    // 过期的状态（例如仍以为这把密钥在冷却中）。
    const billed = await persisted;
    notifyAttempt(onAttempt, {
      keyId: id,
      outcome: 'ok',
      durationMs,
      // 抓取时的「消耗」是累计档位算出来的，不在内核的返回值里；它随落盘一起交回来。
      credits: billed,
